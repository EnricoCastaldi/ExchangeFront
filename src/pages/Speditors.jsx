...
// Type options (backend stores lowercase values)
const TYPES = [
  { value: "spedycja", label: "Spedycja" },
  { value: "dostawcy", label: "Dostawcy" },
  { value: "nabywcy", label: "Nabywcy" },
  { value: "gb", label: "GB" },
];

// Prefix for display: PRZ0000001, PRZ0000002, ...
const SPEDITOR_PREFIX = "PRZ";

function formatSpeditorNo(no) {
  if (no == null || no === "") return null;
  const n = parseInt(no, 10);
  if (Number.isNaN(n)) return String(no);
  return `${SPEDITOR_PREFIX}${String(n).padStart(7, "0")}`;
}

function getTypeLabel(v) {
  if (!v) return "";
  const found = TYPES.find((t) => t.value === v.toLowerCase());
  return found ? found.label : v;
}

export default function Speditors() {
  const { t, locale } = useI18n();
  const P = t?.speditors || {};

  const L = {
    title: P.title || "Speditors (Spedytorzy / kontrahenci)",
    controls: {
      searchPlaceholder:
        P?.controls?.searchPlaceholder ||
        "Search: PRZ no, type",
      searchBtn: P?.controls?.searchBtn || "Search",
      addBtn: P?.controls?.addBtn || "Add speditor",
    ...
    table: {
      ...
      empty: P?.table?.empty || "No speditors",
      dash: P?.table?.dash || "—",
    },
    details: {
      id: P?.details?.id || "ID",
    ...
    modal: {
      titleNew: P?.modal?.titleNew || "Add speditor",
      titleEdit: P?.modal?.titleEdit || "Edit speditor",
      add: P?.modal?.add || "Add",
    ...
    alerts: {
      loadFail: P?.alerts?.loadFail || "Failed to load speditors.",
      requestFail: P?.alerts?.requestFail || "Request failed.",
      deleteConfirm:
        P?.alerts?.deleteConfirm || "Delete this speditor?",
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      let query = q.trim();
      // If user types PRZ0000001 → search by numeric "1"
      if (/^PRZ\d{7}$/i.test(query)) {
      ...
      const res = await fetch(
        `${API}/api/speditors?${params.toString()}`
      );
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/speditors/${id}`, {
        method: "DELETE",
      });
      if (res.status === 204) {
                        ...
                        >
                          <div className="p-4 grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-slate-700">
                            ...
                            <KV
                              ...
                              icon={Hash}
                            >
                              {formatSpeditorNo(r.no) ||
                                L.table.dash}
                            </KV>
                            <KV
                              label={L.details.type}
                              icon={ListFilter}
                            >
                              {getTypeLabel(r.type) ||
                                L.table.dash}
...
/* ---------- Form ---------- */
function SpeditorForm({ initial, onSubmit, onCancel, L }) {
        ...
        {isEdit && (
          ...
          >
            <input
              ...
              value={
                formatSpeditorNo(no) || L.table?.dash || "—"
              }
              readOnly
            />
          </Field>
