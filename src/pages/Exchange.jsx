...
function PriorityBadge({ value }) {
  ...
  return (
    <span
      className={`px-2 py-1 rounded text-xs font-semibold border ${meta.cls}`}
    >
      {meta.label}
    </span>
  );
...
function normalizeBlockRow(r, side = "sell") {
  ...
  return {
    ...
    status: (r.status || "approved").toString().toLowerCase(),
  ...
  const fetchColumn = async (side) => {
    ...
    try {
      ...
      const rows = (json?.data || [])
        .map((r) => normalizeBlockRow(r, side))
 ...
 */
async function buildMatchesHungarian(
  buyRows = [],
  sellRows = [],
  options = {},
) {
  ...
  for (const [itemName, itemBuys] of groupedBuys.entries()) {
    ...
    if (costPerKm > 0) {
      ...
      if (pending.length) {
        await mapLimit(pending, 6, async (p) => {
          try {
            const km = await fetchRoadDistanceKm(
              p.aLat,
              p.aLon,
              p.bLat,
              p.bLon,
    ...
    for (let i = 0; i < m; i++) {
      ...
      const locationBaseCostPerTon = matchedQty
        ? (Number(locationBaseCost) || 0) / matchedQty
        : 0;
      const transportCostPerTon =
        (Number(transportCost) || 0) + (Number(locationBaseCostPerTon) || 0);
...
function MatchesTable({
    ...
    try {
      ...
      popupTimerRef.current = setInterval(() => {
        setPopupSecondsLeft((s) => {
          if (s <= 1) {
            setTimeout(
              () => closePopup({ releaseHold: true, match: matchRow }),
              0,
            );
            return 0;
          }
  ...
  const pickupMatch = async () => {
    ...
    try {
      ...
      setAllMatchesRaw((prev) =>
        prev.filter((m) => !isSameMatch(m, activeMatch)),
      );
      setPrioMatchesRaw((prev) =>
        prev.filter((m) => !isSameMatch(m, activeMatch)),
      );
  ...
  const showHungarian = () => {
    ...
    setPage(1);
    setSort((prev) =>
      prev.key === "prioOrder" ? { key: "spread", dir: "asc" } : prev,
    );
  };
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const res = await buildMatchesHungarian(
          buyAllRows || [],
          sellAllRows || [],
          {
            itemKey,
    ...
    <div className="rounded-2xl border border-slate-200 bg-white relative overflow-visible">
      <div className="px-4 py-3 border-b bg-slate-50">
        ...
        <div className="mt-3 flex flex-wrap items-center gap-2">
          ...
          <span className="text-xs text-slate-500">
            Priority view hides prio=0 matches and enforces unique blocks
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="mt-2 pt-2 border-t border-slate-200 flex justify-between gap-3">
                                    ...
                                    <span className="font-semibold tabular-nums">
                                      {fmtMoney(
                                        d.total ?? r.totalTransportCost ?? 0,
                                        locale,
                                        "PLN",
                                      )}
                                    </span>
            ...
            <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-3">
              <div>
                ...
                <div className="text-xs text-slate-500 mt-1">
                  Buy: {activeMatch.buy_documentNo} (line{" "}
                  {activeMatch.buy_lineNo}) · Sell:{" "}
                  {activeMatch.sell_documentNo} (line {activeMatch.sell_lineNo})
                </div>
              </div>
