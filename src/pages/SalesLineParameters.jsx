// src/pages/SalesLineParameters.jsx
...
export default function SalesLineParameters() {
  const { t, locale, lang } = useI18n();

  // pull translated strings from i18n
  const L = t.salesLineParameters;

  // simple localizations for a couple of placeholders
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${API}/api/sales-line-parameters?${params.toString()}`
      );
  ...
  const onDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API}/api/sales-line-parameters/${id}`, {
        method: "DELETE",
      });
      if (res.status === 204) {
  ...
  const handleSubmit = async (form) => {
    ...
    const url = isEdit
      ? `${API}/api/sales-line-parameters/${id}`
      : `${API}/api/sales-line-parameters`;
...
function SLPForm({
  ...
  // When user picks a param code and value is empty, prefill defaultValue (if available)
  useEffect(() => {
...
function Field({ label, icon: Icon, children }) {
  // detect if the child is a <select>
  const isSelect =
  ...
  // merge classes (preserve any on the child)
  const baseInput =
