// components/PurchaseOfferPdf.jsx
import React, { useEffect, useMemo, useState } from "react";
import pdfMake from "pdfmake/build/pdfmake";
import "pdfmake/build/vfs_fonts"; // Roboto (PL chars)
...
export default function PurchaseOfferPdf({ api, document: doc, lang = "pl" }) {
  const T =
    lang === "en"
      ? {
      ...
      : {
          ...
          dates: "Daty",
          audit: "Audyt",
          buyVendor: "Dostawca (Buy)",
          payVendor: "Płatnik (Pay)",
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        // 1) Load lines for this purchase document
        const params = new URLSearchParams({
          page: "1",
          limit: "500",
          documentNo: doc.documentNo || "",
        });
        const res = await fetch(`${api}/api/purchase-offer-lines?${params.toString()}`);
        ...
        const docDefinition = {
          ...
          content: [
            // red header band
            {
              table: {
                widths: ["*"],
                body: [[{ columns: [{ image: logoDataUrl, width: 120, margin: [8, 6, 0, 6] }, { width: "*", text: "" }], fillColor: "#dc2626" }]],
              },
              layout: "noBorders",
              margin: [-36, -36, -36, 12],
            },
                ...
                { width: 12, text: "" },
                {
                  ...
                  table: {
                    ...
                    body: [
                      ...
                      [{ text: T.validityDate, style: "kvLabel" }, { text: dDate(doc.validityDate || doc.documentValidityDate), style: "kvValue" }],
            ...
            // Audit
            {
              table: {
                ...
                body: [
                  ...
                  [{ text: T.createdBy, style: "kvLabel" }, { text: String(doc.userCreated || "—"), style: "kvValue" }, { text: T.createdAt, style: "kvLabel" }, { text: dDate(doc.dateCreated), style: "kvValue" }],
                  [{ text: T.modifiedBy, style: "kvLabel" }, { text: String(doc.userModified || "—"), style: "kvValue" }, { text: T.modifiedAt, style: "kvLabel" }, { text: dDate(doc.dateModified), style: "kvValue" }],
                ],
              },
              layout: "lightHorizontalLines",
              margin: [0, 0, 0, 10],
            ...
            // Parties
            { text: T.buyVendor, style: "sectionTitle" },
            partyTable(T, {
              no: doc.buyVendorNo,
              name: doc.buyVendorName,
              name2: doc.buyVendorName2,
              address: doc.buyVendorAddress,
              address2: doc.buyVendorAddress2,
              city: doc.buyVendorCity,
              region: doc.buyVendorRegion,
              postCode: doc.buyVendorPostCode,
              country: doc.buyVendorCountry,
              email: doc.buyVendorEmail,
              phone: doc.buyVendorPhoneNo,
              nip: doc.buyVendorNip,
            }),

            { text: T.payVendor, style: "sectionTitle" },
            partyTable(T, {
              no: doc.payVendorNo,
              name: doc.payVendorName,
              name2: doc.payVendorName2,
              address: doc.payVendorAddress,
              address2: doc.payVendorAddress2,
              city: doc.payVendorCity,
              region: doc.payVendorRegion,
              postCode: doc.payVendorPostCode,
              country: doc.payVendorCountry,
              email: doc.payVendorEmail,
              phone: doc.payVendorPhoneNo,
              nip: doc.payVendorNip,
            }),
                  ...
                  table: {
                    ...
                    body: [
                      ...
                      [
                        ...
                        { text: `${fmt(sumLines, 2)} ${doc.currencyCode || ""}`, style: "kvValue", alignment: "right" },
                      ],
                      [
                        { text: T.sumTransport, style: "kvLabel" },
                        { text: `${fmt(sumTransport, 2)} ${doc.currencyCode || ""}`, style: "kvValue", alignment: "right" },
                      ],
                      [
                        { text: T.grandTotal, style: "kvLabel" },
                        { text: `${fmt(grand, 2)} ${doc.currencyCode || ""}`, style: "kvValue", alignment: "right" },
                      ],
                    ],
                  },
                  layout: "lightHorizontalLines",
          ...
          footer: (current, total) => ({
            columns: [
              ...
              { text: `${current} / ${total}`, alignment: "right", style: "small" },
            ],
            margin: [36, 0],
          }),
        };
  ...
  const fileName = useMemo(
    () => `${doc.documentNo || "purchase-offer"}.pdf`,
    [doc.documentNo]
  );
